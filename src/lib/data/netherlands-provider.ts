...
/**
 * Netherlands-specific data provider.
 *
 * Uses netherlandsVerifiedCasinos (verified data, verificationStatus: "verified").
 * Production visibility is gated by verificationStatus === "verified".
 */
export function createNetherlandsProvider(): CasinoDataProvider {
  const casinos = netherlandsVerifiedCasinos;
...
/**
 * Get Netherlands verified casinos.
 * Returns all verified casinos.
 */
export function getNetherlandsFixtures(): Casino[] {
  return [...netherlandsVerifiedCasinos];
}

/**
 * Check if Netherlands has any production-ready casinos.
 */
export function hasNetherlandsProductionData(): boolean {
  return netherlandsVerifiedCasinos.some(c =>
    c.status === "active" && c.verificationStatus === "verified"
  );
}
