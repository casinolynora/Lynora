...
/**
 * Belgium-specific data provider.
 *
 * Uses belgiumVerifiedCasinos (verified data, verificationStatus: "verified").
 * Production visibility is gated by verificationStatus === "verified".
 */
export function createBelgiumProvider(): CasinoDataProvider {
  const casinos = belgiumVerifiedCasinos;
...
/**
 * Get Belgium verified casinos.
 * Returns all verified casinos.
 */
export function getBelgiumFixtures(): Casino[] {
  return [...belgiumVerifiedCasinos];
}

/**
 * Check if Belgium has any production-ready casinos.
 */
export function hasBelgiumProductionData(): boolean {
  return belgiumVerifiedCasinos.some(c =>
    c.status === "active" && c.verificationStatus === "verified"
  );
}
