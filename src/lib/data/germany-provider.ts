...
/**
 * Germany-specific data provider.
 *
 * Uses germanyVerifiedCasinos (verified data, verificationStatus: "verified").
 * Production visibility is gated by verificationStatus === "verified".
 */
export function createGermanyProvider(): CasinoDataProvider {
  const casinos = germanyVerifiedCasinos;
...
/**
 * Get Germany verified casinos.
 * Returns all verified casinos.
 */
export function getGermanyFixtures(): Casino[] {
  return [...germanyVerifiedCasinos];
}

/**
 * Check if Germany has any production-ready casinos.
 */
export function hasGermanyProductionData(): boolean {
  return germanyVerifiedCasinos.some(c =>
    c.status === "active" && c.verificationStatus === "verified"
  );
}
